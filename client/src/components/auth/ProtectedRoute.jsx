...
const ProtectedRoute = ({ children, allowedRoles = [] }) => {
  ...
  if (user.role === "passenger" || user.role === "rider") {
    ...
    const allowedIncompletePaths = isPassenger
      ...
      : [
          ...
          "/rider/vehicle/complete",
        ];

    if (
      !profileComplete &&
      !allowedIncompletePaths.includes(location.pathname)
    ) {
      const destination = isPassenger
  ...
  if (allowedRoles.length > 0 && !allowedRoles.includes(user.role)) {
    const fallback = rolePathPrefix[user.role]
      ? `${rolePathPrefix[user.role]}/dashboard`
      : "/";
