...
const DashboardLayout = ({ children }) => {
  ...
  const userRole =
    user?.role ||
    (["passenger", "rider", "admin"].includes(pathRole)
      ? pathRole
      : "passenger");
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-4 md:px-8 flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div
              className={`relative hidden sm:block group bg-slate-50 rounded-xl transition-all border border-transparent focus-within:border-transparent focus-within:ring-2 ${getThemeColor()}`}
            >
